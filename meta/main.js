...
function processCommits(data) {
    return d3
        ...
        .map(([commit, lines]) => {
            ...
            let ret = {
                id: commit,
                url: "https://github.com/JaredW022/commit/" + commit,
                author,
                date,
                time,
                timezone,
        ...
        .sort((a, b) => a.datetime - b.datetime);   // <-- ADD THIS
}
...
function renderScatterPlot(data, commits) {
    ...
    svg.append('g')
        .attr('transform', `translate(${usableArea.left}, 0)`)
        .attr('class', 'y-axis')   // optional but recommended
        .call(yAxis);
...
function updateFileDisplay(filteredCommits) {
    ...
    // Update the unit visualization inside <dd>
    filesContainer
        .select('dd')
        .selectAll('div')
        .data(d => d.lines)
...
// First render the full scatterplot
renderScatterPlot(data, commits);

// THEN initialize the slider UI
onTimeSliderChange();
